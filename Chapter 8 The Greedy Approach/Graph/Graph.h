...
public:
	...
	double FindShortestPath(int first,int last)
	{
		...
		while(!Y.empty() && std::find(Y.begin(),Y.end(),last) != Y.end())
		{
			...
			{
				if(ShortestLength[*it] != 0 && ShortestLength[*it] < current_value)
				{
					current_value = ShortestLength[*it];
					index = *it;
				}
			}
			X.push_back(index);
			...
			for(auto it = Y.begin(); it != Y.end(); it++)
				if(data[index][*it] != max)
					ShortestLength[*it] = ShortestLength[index] + data[index][*it];
		}
		std::cout << "The shortest path from " << first << " to " << last << " is: " << std::endl;
		std::cout << first;
